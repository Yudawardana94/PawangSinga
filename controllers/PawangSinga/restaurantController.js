...
class RestaurantController {
  ...
  static async findById(req, res, next) {
    ...
    try {
      ...
      if(foundRestaurant.Visibility === "Unlisted") throw {message: "data not found"}
      res.status(200).json({
  ...
  static async search(req,res, next) {
    ...
    try {
      ...
      if(searchValue && searchValue.length === 0) throw {code: 404, message: "Data not found"}
      res.status(200).json({
        data: searchValue,
        status: 200,
        message: "Ok"
  ...
  static async update(req, res, next) {
    ...
    try {
      await restaurantModel.findByIdAndUpdate(restaurantId, payload, {new: true, upsert: true})
      
      res.status(200).json({
        data: {},
        status: 200,
        message: "Restaurant Updated",
        notes: "To remove some tag make unique value from client sidem then on the server side will update based on what client send"
      })
    } catch (error) {
      res.status(500).json(error);
    }
